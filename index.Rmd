...
```
This graph looks at the the entire data set. It shows the years that we are looking at and the unemployment rate in each year. The color corresponds to the majority party in office. This data unemployment data is measured from the Federal Reserve looking at the number of people who are unemployed divided by the entire labor force (the sum of those who are employed and unemployed). This number is then multiplied by 100 to show the percentage of individuals in the country that are unemployed. The data is cross-sectional. 
...
So, this graph plots the unemployment rate of each party over the years. There are two boxes, one for when the Democratic Party is in the majority, and one for when the Republican Party is in the majority. In each box, there is a thin black line. This line represents the median of the entire data set. As seen in the graph, the median unemployment rate when Democrats are in the majority is higher than the median unemployment rate when Republicans are in the majority. 
